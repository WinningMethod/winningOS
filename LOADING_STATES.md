...
Immediate navigation: the shared shell uses `WorkspaceNavigationProvider`, `WorkspaceLink`, and `WorkspaceNavigationContent` to show the destination skeleton on click, before the server response. The router outlet stays mounted while hidden; React's navigation transition controls the handoff. No timers, minimum skeleton duration, extra refresh, or prefetch is used. Same-page filters and Docs topic changes retain their local behavior; modified clicks keep normal link behavior.
