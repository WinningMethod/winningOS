...
This document records what is present, what changed after the recent PRs, and what is still missing before WinningOS Core implementation should proceed.
...
### 2. Data model still had multi-workspace residue
...
### 3. Frontend brief no longer matched the simplified UI

The original V0 brief asked for separate Dashboard, Roles & Permissions, Branding, Agent, and Settings nav items. The accepted wireframe simplified this to Home, Members, and Settings.

Action in this PR:

- update the frontend brief to match the accepted simplified shell
- preserve details as settings sections instead of primary nav items

### 4. Backend implementation order was not explicit enough
...
### 5. Supabase contract needed implementation-level detail
...
### 6. Routes needed permission/data mapping
...
This belongs in the first implementation PR, not necessarily this docs-only PR.
...
### 7. Provider secret strategy

Before real agent configuration, decide:

- where provider API keys live
- whether Supabase Vault is used
- whether only server environment variables are allowed for v0.1
- what is safe to show in browser-visible config

## Recommended next PR after this one

After this documentation PR is reviewed and merged, the next PR should be small implementation groundwork:
...
4. Should `core_agent_settings` exist in the first migration, or wait until provider strategy is clearer?
5. Should audit events be deferred until after the first privileged write actions?
