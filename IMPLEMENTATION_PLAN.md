...
As of this plan, WinningOS Core has:
...
- provider-neutral agent settings placeholder
...
Goal: define the environment variables and client boundaries before schema work.
...
- build passes with placeholder-safe imports
- no server-only env access from client components
...
one placeholder agent settings row, if needed
```
...
- unique constraints prevent accidental second active workspace assumptions
...
4. Keep agent provider settings as a placeholder unless provider implementation is explicitly scoped.
5. Add server-side validation for write actions.
...
5. Use the future meeting-notes plugin only as a validation example.
