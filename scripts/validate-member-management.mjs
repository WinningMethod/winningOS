...
assert(memberData.includes("canInviteMembers"), "member list helper exposes the invite permission")
assert(memberData.includes("canAssignAdmins"), "member list helper exposes the owner-only admin-tier flag")
assert(memberData.includes("canRemoveMembers"), "member list helper exposes owner-only remove permission")
...
assert(memberActions.includes('roleKey === "admin"') && memberActions.includes("invite-admin-owner-only"), "invite action blocks non-owners from inviting admins (issue #60)")
assert(memberActions.includes("resetPasswordForEmail"), "re-inviting an existing auth user sends a set-password email (issue #62)")
assert(memberActions.includes('delivery === "existing" && membershipStatus === "invited"'), "re-invite email only fires when a membership was staged for an existing user")
...
assert(membersPage.includes("canInviteMembers"), "members page uses owner-only invite permission for invite form gate")
assert(membersPage.includes("canRemoveMembers"), "members page uses owner-only remove permission for remove form gate")
assert(membersPage.includes('title={canInviteMembers ? undefined : "Requires the members.invite permission"}'), "members page labels the disabled invite button by its permission")
assert(membersPage.includes("roleOptions(canAssignAdmins)"), "members page hides the Admin option from non-owners (issues #58/#60)")
assert(membersPage.includes('member.roleKey === "admin" && !canAssignAdmins'), "members page hides role controls on admin rows from non-owners")
assert(membersPage.includes("ConfirmForm") && membersPage.includes("Are you sure"), "destructive member actions confirm before submitting (issue #61)")
assert(membersPage.includes("invite-admin-owner-only") && membersPage.includes("reinvite-email-failed") && membersPage.includes("reinvited"), "members page surfaces the admin-tier and re-invite statuses")
assert(membersPage.includes("RemoveMemberForm member={member} canRemoveMembers={canRemoveMembers}"), "members page limits remove action to the remove permission")
